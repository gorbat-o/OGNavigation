...
struct ContentView: View {
    @State private var tabRouter = NavigationTabRouter<AppRoute>(
        tabs: [.homeTab, .dataTab, .accountTab],
        selected: .homeTab
    )

    @State private var isCustomTabBarActivated = false
        ...
        case .accountTab:
            if isCustomIconsActivated {
                Label {
                    ...
                    Image(tabRouter.selected == tab ? "tabAccountIconSelected" : "tabAccountIconUnselected")
                        .renderingMode(.template)
                }
                .foregroundColor(tabRouter.selected == tab ? selectedColor : unselectedColor)
            } else {
                Label("Profile", systemImage: tabRouter.selected == tab ? "person.crop.circle.fill" : "person.crop.circle")
                    .foregroundColor(tabRouter.selected == tab ? selectedColor : unselectedColor)
            }
        default:
    ...
    private func makeCustomTabBar(selected: Binding<AppRoute>, tabs: [AppRoute], isCustomIcons: Binding<Bool>, isCustomColors: Binding<Bool>) -> some View {
        let items: [(tab: AppRoute, item: TabItem)] = [
            ...
            (.accountTab, TabItem(
                ...
                icon: getTabIcon(
                    tab: .accountTab,
                    customSelectedIcon: "tabAccountIconSelected",
                    customUnselectedIcon: "tabAccountIconUnselected",
                    systemSelectedIcon: "person.crop.circle.fill",
                    systemUnselectedIcon: "person.crop.circle"
                )
            ))
        ]
